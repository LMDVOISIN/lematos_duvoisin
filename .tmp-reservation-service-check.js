...
if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error("Variables d'environnement Supabase manquantes. Verifiez votre fichier .env pour VITE_SUPABASE_URL et VITE_SUPABASE_ANON_KEY");
}
...
async function invokeSendEmailDirect(functionBody) {
  ...
  try {
    const response = await fetch(`${supabaseUrl2.replace(/\/$/, "")}/functions/v1/send-email`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    if (!response.ok) {
      return {
        data: null,
        error: {
          message: parsedBody?.error || parsedBody?.message || rawText || `Edge Function HTTP ${response.status}`
        }
      };
    }
  ...
  try {
    ...
    if (resolvedTemplateKey) {
      ...
      const loadTemplate = async (idColumn) => {
        ...
        let localError = null;
        ({ data: localTemplate, error: localError } = await supabase?.from("email_templates")?.select("subject, body_html, enabled")?.eq(idColumn, resolvedTemplateKey)?.single());
        if (localError && /column\s+email_templates\.enabled\s+does not exist/i.test(String(localError?.message || ""))) {
          ({ data: localTemplate, error: localError } = await supabase?.from("email_templates")?.select("subject, body_html")?.eq(idColumn, resolvedTemplateKey)?.single());
        }
    ...
    if (!to || !finalSubject || !finalHtmlBody) {
      const error2 = "Champs requis manquants : destinataire, sujet et corps HTML";
      console.error(error2);
      await logFailedEmail(to, resolvedTemplateKey, resolvedVariables, error2, finalSubject);
...
var reservationService = {
  ...
  createReservation: async (reservationData) => {
    try {
      ...
      const { data, error } = await supabase?.from("reservations")?.insert(dataToInsert)?.select()?.single();
      if (error) {
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans createReservation:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
      const { data: annonce } = await supabase?.from("annonces")?.select("titre, owner_id, prix_jour, caution")?.eq("id", data?.annonce_id)?.single();
      const { data: renterProfile } = await supabase?.from("profiles")?.select("pseudo, email")?.eq("id", user?.id)?.single();
      const { data: ownerProfile } = await supabase?.from("profiles")?.select("pseudo, email")?.eq("id", annonce?.owner_id)?.single();
      ...
      const duration = Math.ceil((new Date(data.end_date) - new Date(data.start_date)) / (1e3 * 60 * 60 * 24));
      if (ownerProfile?.email) {
        await sendEmail({
          to: ownerProfile?.email,
          templateKey: "reservation_new_owner",
          variables: {
            owner_name: ownerProfile?.pseudo || "Propri\xE9taire",
            renter_name: renterProfile?.pseudo || "Locataire",
            annonce_title: annonce?.titre || "\xC9quipement",
            start_date: startDate,
            end_date: endDate,
            duration: duration?.toString(),
            total_price: data?.total_price || "0",
            caution: annonce?.caution || "0",
            reservation_url: `${window.location?.origin}/tableau-gestion-reservations`
          }
        });
      }
      if (renterProfile?.email) {
        await sendEmail({
          ...
          templateKey: "reservation_received_renter",
          variables: {
            renter_name: renterProfile?.pseudo || "Locataire",
            annonce_title: annonce?.titre || "\xC9quipement",
            start_date: startDate,
  ...
  getReservationById: async (id) => {
    try {
      const { data, error } = await supabase?.from("reservations")?.select(`
          *,
          annonce:annonces(titre, photos, prix_jour, caution, owner_id),
      ...
      if (error) {
        if (error?.code === "PGRST116")
          return { data: null, error: null };
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans getReservationById:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
  ...
  getUserReservations: async (userId, filters = {}) => {
    try {
      let query = supabase?.from("reservations")?.select(`
          *,
          annonce:annonces(titre, photos, prix_jour, city)
        `)?.eq("renter_id", userId);
      if (filters?.status)
      ...
      if (error) {
        if (error?.code === "PGRST116")
          return { data: [], error: null };
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans getUserReservations:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
  ...
  /**
   * Get owner's reservations (as owner)
   */
  getOwnerReservations: async (ownerId, filters = {}) => {
    try {
      let query = supabase?.from("reservations")?.select(`
          *,
          annonce:annonces(titre, photos, prix_jour, caution),
          renter:profiles!reservations_renter_id_fkey(pseudo, avatar_url, email, phone)
        `)?.eq("owner_id", ownerId);
      if (filters?.status)
        query = query?.eq("status", filters?.status);
      ...
      if (error) {
        if (error?.code === "PGRST116")
          return { data: [], error: null };
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans getOwnerReservations:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
  ...
  updateReservationStatus: async (id, newStatus, additionalData = {}) => {
    try {
      ...
      if (error) {
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans updateReservationStatus:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
      const { data: fullReservation } = await supabase?.from("reservations")?.select(`
        *,
        annonce:annonces(titre, prix_jour, caution),
        renter:profiles!reservations_renter_id_fkey(pseudo, email),
        owner:profiles!reservations_owner_id_fkey(pseudo, email)
      `)?.eq("id", id)?.single();
      if (!fullReservation)
        return { data, error: null };
      ...
      if (newStatus === "accepted") {
        if (fullReservation?.renter?.email) {
          await sendEmail({
            ...
            variables: {
              ...
              owner_name: fullReservation?.owner?.pseudo || "Propri\xE9taire",
              annonce_title: fullReservation?.annonce?.titre || "\xC9quipement",
              start_date: startDate,
              end_date: endDate,
        ...
        const cancellationReason = additionalData?.cancellation_reason || "Non sp\xE9cifi\xE9e";
        if (fullReservation?.renter?.email) {
          await sendEmail({
            to: fullReservation?.renter?.email,
        ...
        if (fullReservation?.owner?.email) {
          await sendEmail({
            ...
            variables: {
              ...
              start_date: startDate,
              end_date: endDate,
              annonce_url: `${window.location?.origin}/tableau-gestion-reservations`,
              cancellation_reason: cancellationReason
            }
          });
        }
        ...
        if (fullReservation?.owner?.email) {
          const ownerAmount = (parseFloat(fullReservation?.total_price || 0) * 0.85)?.toFixed(2);
          await sendEmail({
            to: fullReservation?.owner?.email,
        ...
        if (fullReservation?.renter?.email) {
          await sendEmail({
            ...
            variables: {
              ...
              annonce_title: fullReservation?.annonce?.titre || "\xC9quipement",
              caution: fullReservation?.annonce?.caution || "0",
              review_url: `${window.location?.origin}/tableau-gestion-reservations`
            }
          });
        }
        if (fullReservation?.owner?.email) {
  ...
  acceptReservation: async (id) => {
    return await reservationService?.updateReservationStatus(id, "accepted", {
      owner_confirmed_at: (/* @__PURE__ */ new Date())?.toISOString()
    });
  },
  /**
   * Cancel reservation
  ...
  refuseReservation: async (id, refusalReason) => {
    try {
      ...
      if (error) {
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans refuseReservation:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
      ...
      if (fullReservation?.renter?.email) {
        await sendEmail({
          ...
          variables: {
            ...
            owner_name: fullReservation?.owner?.pseudo || "Propri\xE9taire",
            annonce_title: fullReservation?.annonce?.titre || "\xC9quipement",
            refusal_reason: refusalReason || "Non sp\xE9cifi\xE9e",
            search_url: `${window.location?.origin}/accueil-recherche`
          }
        });
      }
  ...
  updatePaymentInfo: async (id, paymentData) => {
    try {
      ...
      if (error) {
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans updatePaymentInfo:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
  ...
  markPaymentCompleted: async (id, stripePaymentIntentId) => {
    try {
      const { data, error } = await supabase?.from("reservations")?.update({
        ...
        tenant_payment_paid_at: (/* @__PURE__ */ new Date())?.toISOString(),
        updated_at: (/* @__PURE__ */ new Date())?.toISOString()
      })?.eq("id", id)?.select()?.single();
      if (error) {
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans markPaymentCompleted:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
  ...
  updateDepositStatus: async (id, depositStatus, options = {}) => {
    try {
      ...
      if (depositStatus === "released") {
        updateData.deposit_released_at = (/* @__PURE__ */ new Date())?.toISOString();
      }
      ...
      if (error) {
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans updateDepositStatus:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
  ...
  updateDocumentStatus: async (id, documentType, uploaded = true) => {
    try {
      ...
      if (!fields)
        return { data: null, error: { message: "Invalid document type" } };
      const updateData = {
        [fields?.uploaded]: uploaded ? (/* @__PURE__ */ new Date())?.toISOString() : null,
        updated_at: (/* @__PURE__ */ new Date())?.toISOString()
      };
      const { data, error } = await supabase?.from("reservations")?.update(updateData)?.eq("id", id)?.select()?.single();
      if (error) {
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans updateDocumentStatus:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
    ...
    try {
      ...
      if (error) {
        if (error?.code === "PGRST116")
          return { data: [], error: null };
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans getReservationsByAnnonce:", error?.message);
          throw error;
        }
        return { data: null, error };
      }
  ...
  checkAvailability: async (annonceId, startDate, endDate) => {
    try {
      const { data, error } = await supabase?.from("reservations")?.select("id, start_date, end_date")?.eq("annonce_id", annonceId)?.in("status", ["pending", "accepted", "paid", "active"])?.or(`start_date.lte.${endDate},end_date.gte.${startDate}`);
      if (error) {
        if (error?.code === "PGRST116")
          return { available: true, error: null };
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans checkAvailability:", error?.message);
          throw error;
        }
   ...
   */
  hasOngoingReservationLinkBetweenUsers: async ({ userId, otherUserId } = {}) => {
    try {
      if (!userId || !otherUserId) {
        return { data: false, error: null };
      }
      const activeStatuses = ["pending", "accepted", "paid", "active", "ongoing"];
      const left = String(userId);
      const right = String(otherUserId);
      const { count, error } = await supabase?.from("reservations")?.select("id", { count: "exact", head: true })?.in("status", activeStatuses)?.or(`and(owner_id.eq.${left},renter_id.eq.${right}),and(owner_id.eq.${right},renter_id.eq.${left})`);
      if (error) {
        if (isSchemaError(error)) {
          console.error("Erreur de sch\xE9ma dans hasOngoingReservationLinkBetweenUsers:", error?.message);
          throw error;
        }
...
var reservationService_default = reservationService;
export {
  reservationService_default as default
};
