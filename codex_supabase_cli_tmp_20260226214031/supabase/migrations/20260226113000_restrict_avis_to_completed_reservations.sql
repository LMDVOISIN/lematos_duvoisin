...
BEGIN
  ...
  IF v_auth_role <> 'service_role' THEN
    ...
    IF NEW.reviewer_id <> v_auth_user_id THEN
      RAISE EXCEPTION 'reviewer_id doit correspondre a l utilisateur connecte';
    END IF;
  END IF;
  ...
  IF v_status NOT IN (
    ...
    RAISE EXCEPTION 'Un avis ne peut etre depose qu apres une reservation terminee';
  END IF;
  ...
  IF NEW.reviewer_id = v_reservation.owner_id AND NEW.reviewed_user_id <> v_reservation.renter_id THEN
    RAISE EXCEPTION 'Le proprietaire ne peut noter que le locataire de cette reservation';
  END IF;

  IF NEW.reviewer_id = v_reservation.renter_id AND NEW.reviewed_user_id <> v_reservation.owner_id THEN
    RAISE EXCEPTION 'Le locataire ne peut noter que le proprietaire de cette reservation';
  END IF;

  IF NEW.annonce_id IS NULL THEN
    NEW.annonce_id := v_reservation.annonce_id;
  ELSIF NEW.annonce_id <> v_reservation.annonce_id THEN
    RAISE EXCEPTION 'annonce_id doit correspondre a la reservation referencee';
  END IF;
