...
-- Compatibility guard: support projects where email_templates uses template_key
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
  ...
  IF EXISTS (
    ...
    CREATE OR REPLACE FUNCTION public.sync_email_templates_template_key_from_key()
    RETURNS trigger
    LANGUAGE plpgsql
    AS $fn$
    BEGIN
      IF NEW.template_key IS NULL OR btrim(NEW.template_key) = '' THEN
        NEW.template_key := NEW.key;
      END IF;
      IF NEW.key IS NULL OR btrim(NEW.key) = '' THEN
        NEW.key := NEW.template_key;
      END IF;
      RETURN NEW;
    END;
    $fn$;

    DROP TRIGGER IF EXISTS trg_sync_email_templates_template_key_from_key ON public.email_templates;
    CREATE TRIGGER trg_sync_email_templates_template_key_from_key
      BEFORE INSERT OR UPDATE ON public.email_templates
      FOR EACH ROW
      EXECUTE FUNCTION public.sync_email_templates_template_key_from_key();
  ...
  IF EXISTS (
    ...
    IF NOT EXISTS (
      ...
      WHERE schemaname = 'public'
        ...
        AND indexname = 'idx_email_templates_key_unique'
    ) THEN
      CREATE UNIQUE INDEX idx_email_templates_key_unique
...
(
  ...
  $b29$<h1>Paiement confirmé</h1><p>Le paiement de {{amount}}€ pour la location de {{item_title}} a été reçu.</p><p>Vous recevrez {{owner_net}}€ apr?s deduction des frais applicables (commission plateforme et frais techniques de caution le cas echeant).</p>$b29$,
  true
),
