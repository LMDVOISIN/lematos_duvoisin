...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE n.nspname = 'public'
      AND t.typname = 'test_scenario_program_family'
  ) THEN
    CREATE TYPE public.test_scenario_program_family AS ENUM (
      'successful',
      'renter_failure',
      'owner_failure'
    );
...
ALTER TABLE public.test_scenarios
  ADD COLUMN IF NOT EXISTS program_family public.test_scenario_program_family;

CREATE INDEX IF NOT EXISTS idx_test_scenarios_program_family
  ON public.test_scenarios(program_family);

CREATE INDEX IF NOT EXISTS idx_test_scenarios_program_family_active
  ON public.test_scenarios(program_family, is_active);

ALTER TABLE public.test_mirror_rounds
  ADD COLUMN IF NOT EXISTS program_family public.test_scenario_program_family;

CREATE INDEX IF NOT EXISTS idx_test_mirror_rounds_campaign_program_family
  ON public.test_mirror_rounds(campaign_id, program_family);
...
WHERE rounds.reference_scenario_id = scenarios.id
  AND rounds.program_family IS NULL
  AND scenarios.program_family IS NOT NULL;
...
DECLARE
  ...
  v_progress_count integer := 0;
  v_total_required_tests integer := 3;
...
BEGIN
  ...
  v_total_required_tests := COALESCE((v_progress ->> 'totalRequiredTests')::integer, 3);
